...
test_that("Errors are thrown", {
  expect_error(dborel(x = 0, mu = 0.5),
               "'x' must be greater than 0"
               )
  expect_error(dborel(x = 1, mu = -0.5),
               "'mu' must be greater 0 but less than Inf"
               )
  expect_error(dborel(x = 1, mu = Inf),
               "'mu' must be greater 0 but less than Inf"
  )
  expect_error(rborel(n = 0, mu = -0.5),
               "'mu' must be greater 0 but less than Inf"
               )
  expect_error(rborel(n = 0, mu = Inf),
               "'mu' must be greater 0 but less than Inf"
  )
})
